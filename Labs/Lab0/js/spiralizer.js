...
function setup(){
    ...
    //Count of particles in particleYSpans is dictated by the same particleCount
    particleYSpans = new Array(particleCount);
}
...
function buildSineWave(waveAmp,wavePhaseShift = 0){
    ...
    for (let particle = 0; particle < particleCount; particle++){
        ...
        //Particle's Y span is roughly determined by comparing it to the previous particle, it almost maps the instantaenous change as the particle's height
        //Small Constant added so it doesn't go totally to zero
        particleYSpans[particle] = Math.abs(particleYDisplacements[particle] - particleYDisplacements[particle-1]) + 0.1;
...
function renderWave(particleSize,particleR=255,particleG=255,particleB=255){
    ...
    for (let particle = 0; particle < particleCount; particle++){
        ...
        ellipse(particle*particleSpacing,height/2 + particleYDisplacements[particle], particleSize,particleYSpans[particle]);
    }
